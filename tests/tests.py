...
class TestCityRoute(unittest.TestCase):
    def test_fetch_private_ip_info(self):
        ip: str = "192.168.0.2"
        response = client.get(f"/city/{ip}")
        assert response.status_code == 400
        assert response.json() == {
            "detail": "The IP address '192.168.0.2' is a reserved IP address (private, multicast, etc.)."}
    ...
    def test_fetch_valid_ip(self):
        ...
        response = client.get(f"/city/{ip}")
        assert response.status_code == 200
        assert response.json().get("country") is not None  # Проверяем страну, потому что geolite может не найти город


class TestFullRoute(unittest.TestCase):
    def test_fetch_private_ip_info(self):
        ip: str = "192.168.0.3"
        response = client.get(f"/full/{ip}")
        assert response.status_code == 400
        assert response.json() == {
            "detail": "The IP address '192.168.0.3' is a reserved IP address (private, multicast, etc.)."}
    ...
    def test_fetch_valid_ip(self):
        ...
        response = client.get(f"/full/{ip}")
        assert response.status_code == 200
        assert response.json().get("time_zone") is not None
